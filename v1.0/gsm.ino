   ...
   void gsm_setup()
    {
      ...
      digitalWrite(C_PWR_GSM, LOW); 
      digitalWrite(C_KILL_GSM, LOW); 
      
      pinMode(C_PWR_GSM, OUTPUT);
      pinMode(C_KILL_GSM, OUTPUT);         
      
      pinMode(STATUS_GSM, INPUT);
      pinMode(RING_GSM, INPUT); 
      pinMode(WAKE_GSM, INPUT);         
 ...
 void gsm_restart()
  {
      ...
      for (int i = 0; i < 5; i++) 
       {
        ...
        digitalWrite(LED_BUILTIN, ledState);   // set the LED on
        delay(200);
       }
    ...
    //check if modem is ON (PWRMON is HIGH)
     int pwrmon = digitalRead(STATUS_GSM);
     if(pwrmon == HIGH)
       {
      ...
      for(i=0; i<strlen(tmpval); i++)
        {
          ...
          if(i > 18)  //time can not exceed 20 chars
            {
              break; 
            } 
        }
  ...
  void gsm_get_imei()
    {     
      ...
      for(i=0; i<strlen(tmpval); i++)
        {
          imei[i] = tmpval[i];            
          if(i > 18)  //imei can not exceed 20 chars
            {
              break; 
            } 
        }
  ...
  void gsm_send_http_current()
    {
      ...
      //this will send Current data
       ...
       int http_len = strlen(imei)+strlen(config.key)+strlen(data_current);
